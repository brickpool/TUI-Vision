...
sub as_bios {
  my ( $cell ) = @_;
  return ( $cell->getAttr()->asBIOS() << 8 ) | ord( $cell->getChar()->getText() );
}
...
is( as_bios( $buffer->[0] ), 0x1f00, 'putAttribute sets the attribute correctly' );

# Test putChar method
can_ok( $buffer, 'putChar' );
$buffer->putChar( 0, 'A' );
is( as_bios( $buffer->[0] ), 0x1f41, 'putChar sets the character correctly' );
...
is( as_bios( $buffer->[0] ), 0x2f48, 'moveCStr moves the string correctly' );
is( as_bios( $buffer->[5] ), 0x1f57, 'moveCStr toggles the attribute correctly' );
