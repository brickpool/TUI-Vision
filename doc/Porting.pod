...
This document describes the porting of Borland Turbo Vision to Perl.
It focuses on implementation status, class mapping and technical
considerations arising from differences between C++ and Perl.

This document is primarily intended for maintainers and contributors.

The TUI framework is based on the original Turbo Vision 2.0 C++ sources.
The goal of the port is conceptual compatibility rather than a
mechanical translation of code.

Turbo Vision classes are mapped to Perl modules in a mostly one-to-one
fashion.
...
=head2 Class Implementation Status

The following table provides an overview of the implementation status
of major Turbo Vision classes in the Perl port. The status reflects
practical usability rather than theoretical completeness.
...
The status table is intended as a practical guide rather than a strict
feature checklist.
...
The class does not exist in the Perl port. References to this class are
documented for transparency and orientation only. No compatibility is
provided.

=item [x] Implemented

The class is usable for typical applications. Core functionality is
present and stable. Some optional or rarely used features may still be
missing, but the class can be relied upon in normal use.

=item [o] Partially implemented

The class exists and provides limited functionality. Common use cases
may work, but parts of the API, platform-specific behavior or advanced
features are missing or incomplete. Use with care.

=item [+] Port extension

The class is specific to the Perl port and has no direct equivalent in the
original Turbo Vision implementation. It was introduced to support
additional functionality provided by modern terminals, color handling or
the Perl implementation architecture.

=back

The table focuses on practical relevance. A class marked as implemented
does not imply full parity with the original C++ version in every
detail.
...
=head3 Color and Screen Primitives

 Class              Perl Module                  Status
 ------------------ ---------------------------- ------
 TColorAttr         TUI::Drivers::ColorAttr      [+]
 TColor             TUI::Drivers::Color          [+]
 TAttrPairs         TUI::Drivers::AttrPairs      [+]
 TCellChar          TUI::Drivers::CellChar       [+]
 TScreenCell        TUI::Drivers::ScreenCell     [+]
...
=head3 Driver Layer
 ...
 TScreen            TUI::Drivers::Screen         [x]
 TDisplay           TUI::Drivers::Display        [x]
 THardwareInfo      TUI::Drivers::HardwareInfo   [x]
...
=head3 Standard Dialogs

 Class              Perl Module                  Status
 ------------------ ---------------------------- ------
 ffblk/find_t       TUI::StdDlg::Dos             [x]
 FindFirstRec       TUI::StdDlg::FindFirstRec    [x]
...
=head3 Color Selection Dialog

 Class              Perl Module                   Status
 ------------------ ----------------------------- ------
 TColorDisplay      TUI::ColorSel::ColorDisplay   [x]
 TColorGroup        TUI::ColorSel::ColorGroup     [x]
 TColorGroupList    TUI::ColorSel::ColorGroupList [x]
 TColorIndex        TUI::ColorSel::ColorGroup     [x]
 TColorItem         TUI::ColorSel::ColorItem      [x]
 TColorItemList     TUI::ColorSel::ColorItemList  [x]
 TColorSelector     TUI::ColorSel::ColorSelector  [x]
 TClockView         TUI::ColorSel::ClockView      [x]
 TMonoSelector      TUI::ColorSel::MonoSelector   [x]
...
=head3 Validator System
 ...
 Class                   Perl Module                          Status
 ----------------------- ------------------------------------ ------
 TValidator              TUI::Validate::Validator             [x]
 TFilterValidator        TUI::Validate::FilterValidator       [x]
 TRangeValidator         TUI::Validate::RangeValidator        [x]
 TLookupValidator        TUI::Validate::LookupValidator       [x]
 TStringLookupValidator  TUI::Validate::StringLookupValidator [x]
 TPXPictureValidator     TUI::Validate::PXPictureValidator    [x]
...
=head3 Collections Framework
 ...
 TLineCollection      TUI::Gadgets::LineCollection      [x]
...
=head3 Editors and Text Devices
...
=head3 Gadgets
 ...
 TClickTester       TUI::Gadgets::ClickTester    [x]
 TClockView         TUI::Gadgets::ClockView      [x]
 TEventViewer       TUI::Gadgets::EventViewer    [x]
 TFileViewer        TUI::Gadgets::FileViewer     [x]
 TFileWindow        TUI::Gadgets::FileWindow     [x]
 THeapView          TUI::Gadgets::HeapView       [x]
 TMouseDialog       TUI::Gadgets::MouseDialog    [x]
 TPuzzleView        (missing)                    [ ]

=head2 Memory Management and Resource Handling
...
C<NoteSV> and C<CheckSV> are two functions from the L<Devel::Leak> module that 
can be used to track memory leaks in Perl applications. They work together to 
identify and report on memory that has been allocated but not properly released.

    my $count = Devel::Leak::NoteSV($handle);
    my $count = Devel::Leak::CheckSV($handle);

C<NoteSV> searches the Perl-internal table of assigned scalar values (which 
...
C<CheckSV> is passed an argument containing a reference to a table created by 
C<NoteSV>. It runs through the Perl internals again and calls C<sv_dump()> for 
all I<things> that were not yet present when C<NoteSV> was called. It returns 
...
Turbo Vision Pascal implementation of I<SetMemTop>.
