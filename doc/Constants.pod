...
The Turbo Vision framework defines a large set of constants used to
describe behavior rather than encode logic.

Constants are used for palettes, flags, command identifiers and event
classification. They are central to the framework because many design
decisions are expressed declaratively through constant values.

Understanding the naming scheme is usually more important than
memorizing individual values.

=head2 Naming Conventions

Turbo Vision follows strict naming conventions to make source code
...
Type names begin with an uppercase C<T>.

=item 2.

Registration records for stream I/O begin with an uppercase C<R>.

=item 3.

Identifiers starting with lowercase letters denote constants. The
prefix identifies the functional area of the constant.

=back

The following table lists the standard constant prefixes used
throughout the framework:
  ...
  cs      Color Enumeration       csBackground
  ct      Color types             ctBIOS
...
The C<bfXXXX> values are passed as a bit mask to C<< TButton->new >> and specify 
button attributes, as shown in the table:
...
B<Note>: It's up to the programmer to ensure that each C<TGroup> entity 
(C<TDialog> is descended from C<TGroup>) has only one default button.
...
passed to C<handleEvent> methods in the C<< $event->{command} >> field. 

Constant values of C<0..99>, and C<256..999> are reserved for use by 
Turbo Vision. Your applications can define command constants in the range 
C<100..255>, and C<1000..65535>.
  ...
  cmScreenChanged     57  When the screen size changes, this message is sent to 
                          the application to allow it to resize and redraw its 
                          views.
...
When a C<TCollection> error occurs, the method C<< TCollection->error >> is 
called, with the Code parameter having one of the values shown in the table.
...
Desired color format constants

Use the constants from the table below to select the color representation
stored in a C<TColor> value.

  Constant   Value   Usage
  ---------- ------- --------------------------------------
  ctDefault  0x0000  Terminal default color
  ctBIOS     0x0001  BIOS color attribute
  ctRGB      0x0002  24-bit RGB color
  ctXTerm    0x0003  XTerm 256-color palette index
  
=head2 csXXXX

Color selection constants

The C<csXXXX> constants are used to specify color roles within the UI, such as 
background and foreground colors.

  Constant        Value   Usage
  --------------- ------- ---------------------------------------------------
  csBackground    0x0001  Background color
  csForeground    0x0002  Foreground color

=head2 dmXXXX

Drag Mode constants

The C<< TView->dragMode >> field controls whether or not a view can be dragged 
or have its size changed. The C<dragMode> field is stored in a scalar, with the 
bits set by the constants shown in the following table, and C<$limits> is a 
C<TRect> parameter to C<< TView->dragView >>.
...
B<See>: C<positionalEvents> and C<focusedEvents>.

=head2 focusedEvents

    use constant focusedEvents => evKeyboard | evCommand;

C<focusedEvents> contains a bit mask for identifying events that are focused. 
Any event that is neither a focused event nor a positional event, is classed as 
a broadcast event. 

=head2 gfXXXX

C<< TView->growMode >> field constants

The C<gfXXXX> constants set the C<< TView->growMode >> field, which controls 
how a view grows in relation to the view that owns it.
...
The C<kbXXXX> constants are divided into two groups: a small group for 
...
C<< TEvent->{keyCode} >> field. For example,
...
C<maxCollectionSize> sets the maximum number of elements that can be put into a
collection.
...
The C<mbXXXX> constants are used to test the C<< TEvent->{buttons} >> field of 
a C<TEvent> hash to determine if the left or right button was pressed.
...
B<Note>: C<messageBox> only works within Turbo Vision applications. Do not 
attempt to use it in non-Turbo Vision applications.

B<See>: C<sprintf>, C<messageBoxRect>, C<TStaticText>
...
that route the same as a positional C<$event>.

B<See>: C<focusedEvents>
...
C<< TScrollBar->scrollStep >>. The last 3 C<sbXXXX> constants, C<sbHorizontal>, 
...
C<< TWindow->standardScrollBar >> method.
...
B<See>: C<blockCursor>, C<exposed>, C<hide>, C<hideCursor>,
C<normalCursor>, C<setState>, C<show>, C<showCursor> from C<TView>.

=head2 slXXXX

Style attribute constants

Use the constants from the table below to set or test style attributes in
a C<TColorAttr> value.

  Constant     Value   Usage
  ------------ ------- --------------------------------------------
  slBold       0x001   Bold text
  slItalic     0x002   Italic text
  slUnderline  0x004   Underlined text
  slBlink      0x008   Blinking text
  slReverse    0x010   Reverse foreground and background
  slStrike     0x020   Strikethrough text
  slNoShadow * 0x200   Suppress window shadow rendering
...
modes. 
  ...
  smColor256  0x0200  256-color mode
  smColorHigh 0x0400  High color mode
  smUpdate    0x8000  Update the screen immediately

B<See>: C<< TScreen->setVideoMode >>, C<< TScreen->screenMode >>, 
C<< TProgram->setScreenMode >>
...
When ever a C<< TWindow->number >> field contains this value, it means that the
...
Alt-number quick key selection for switching between windows numbered C<1>
through C<9>.
