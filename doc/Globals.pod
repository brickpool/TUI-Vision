...
=item * A grouped overview for orientation

=item * A complete alphabetical reference table

=back

The alphabetical table is the authoritative source.

=head1 GROUPED OVERVIEW

This section provides a conceptual overview of important groups of global
variables. Each entry includes a short description to give immediate
context. For a complete and authoritative inventory, see the
alphabetical reference table below.

=head2 Application and Desktop

These variables represent the global application state and the top-level
UI containers. They define the structural backbone of a running TUI
application.
...
=head2 Screen and Display

These variables describe the current screen configuration, cursor
behavior and low-level display state. They are maintained by the screen
driver and affect how views are rendered and updated.
...
=head2 Event and Input Handling

This group tracks the current input state and supports event processing.
Most variables are maintained by the event queue and input drivers.
...
These variables reflect the availability and configuration of mouse
hardware and drivers. They allow the framework to adapt to different
hardware capabilities.
 ...
 $mouseRangeX        Maximum X coordinate for mouse events
 $mouseRangeY        Maximum Y coordinate for mouse events

=head2 Dialogs and Text Resources

This group contains shared text resources used by dialogs and message
boxes. Centralizing these variables ensures consistent wording across
the user interface.
 ...
 $validSignedChars   Validator pattern for signed numeric input
 $validUnsignedChars Validator pattern for unsigned numeric input

=head2 Platform Backends

These variables are maintained by platform-specific backend
implementations and are not part of the portable framework API.

 $insertState        Insert/overwrite mode state
 $platform           Platform identifier string
 $pendingEvent       Pending backend event flag

 $lastButtons        Previous mouse button state
 $downButtons        Mouse button state at button press
 $downTicks          Tick count recorded at button press
 $doubleDelay        Maximum double-click interval
 $lastDouble         Double-click detection state

 @lastWhere          Coordinates of the previous mouse action
 @downWhere          Coordinates where the button was pressed

=head1 COMPLETE ALPHABETICAL REFERENCE

The following table lists all global variables used across the framework,
sorted alphabetically by variable name. This table is the authoritative
inventory.
  ...
  @downWhere           HardwareInfo::Termbox  Mouse press coordinates
  @irBuffer            HardwareInfo::Win32    Input record buffer
  @lastWhere           HardwareInfo::Termbox  Last mouse action coordinates
  ...
  $bakText             TColorDialog           Color dialog background label
  $button              TCheckBoxes            Checkbox item pattern
  $button              TMonoSelector          Button item pattern
  ...
  $cancelText          TColorDialog           Cancel button label
  ...
  $colors              TColorDialog           Color selection label
  $colorText           TColorDialog           Color selection text label
  ...
  $doubleDelay         HardwareInfo::Termbox  Double-click delay
  $downButtons         HardwareInfo::Termbox  Mouse button state at press time
  $downMouse           TEventQueue            Mouse-down state
  $downTicks           TEventQueue            Ticks since mouse press
  $downTicks           HardwareInfo::Termbox  Tick count at mouse press
  ...
  $errorMsg            TFilterValidator       Filter validation error message
  $errorMsg            TPXPictureValidator    Picture validation error message
  $errorMsg            TRangeValidator        Range validation error message
  $errorMsg            TStringLookupValidator String lookup validation message
  ...
  $forText             TColorDialog           Foreground color label
  ...
  $groupText           TColorDialog           Color group label
  ...
  $highlight           TMonoSelector          Highlighted text attribute
  ...
  $icon                TColorSelector         Color selector icon
  ...
  $insertState         HardwareInfo::*        Insert/overwrite input mode
  ...
  $inverse             TMonoSelector          Inverse text attribute
  $itemText            TColorDialog           Color item label

  $lastButtons         HardwareInfo::Termbox  Previous mouse button state
  $lastDir             TDirListBox            Last directory tree element
  $lastDouble          HardwareInfo::Termbox  Double-click detection state
  ...
  $maxHeapSize         TUI::Memory            Maximum total cache entries
  ...
  $mouseRangeX         THWMouse               Maximum mouse X coordinate 
  $mouseRangeY         THWMouse               Maximum mouse Y coordinate
  $mouseReverse        TEventQueue            Reversed mouse buttons flag

  $noMouse             THWMouse               Mouse hardware unavailable flag
  $normal              TMonoSelector          Normal text attribute

  $okText              MsgBoxText             OK button label
  $okText              TChDirDialog           OK button label
  $okText              TColorDialog           OK button label
  ...
  $pendingEvent        HardwareInfo::*        Buffered input event indicator
  $platform            HardwareInfo::*        Platform identifier string
  ...
  $textText            TColorDialog           Color preview label
  $tooManyFiles        TFileList              Too many files message

  $underline           TMonoSelector          Underlined text attribute
  $unZoomIcon          TFrame                 Unzoom window icon

  $vChars              TScrollBar             Vertical scrollbar characters
  $validSignedChars    TRangeValidator        Validator signed numeric pattern
  $validUnsignedChars  TRangeValidator        Validator unsigned numeric pattern
...
L<TUI::Manual::Reference::Constants>,
L<TUI::Manual::Reference::Events>,
L<TUI::Manual::Reference::Screen>
