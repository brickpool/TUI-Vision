...
=head2 Key Features of TUI-Vision
...
functionality of TUI-Vision easily and efficiently. The L<UNIVERSAL::Object> 
package (by default), L<Moo>, L<Moos> and L<Moose> are currently supported. 
...
The default toolkit and L<Moos> is compatible for our API and implemented 
in pure Perl. 

It's up to you what you want to use. Simply C<use Moo> in your application, 
instead of C<use TUI::toolkit> if you want to use L<Moo>. TUI-Vision will then 
use L<Moo> in its entirety (the same applies to L<Moose> and L<Moos>, of 
course). 
